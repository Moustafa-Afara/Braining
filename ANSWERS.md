...
## Part 20 · قرارات ٥ أيلول ٢٠٢٦ — نطاق M7 وسياسة تقارير M8

### M7-1 · مجلّد العمل: **كل المجلّدات** — مع قائمة منع قصيرة يفرضها حكمُ المالك نفسه

سُئل عن المجلّد الذي يعمل فيه الوكيل فأجاب: **«كل المجلدات حرفياً»**. هذا توسيعٌ صريح للحارس الثالث
في `BRAINING.md` §7، والذي ينصّ نفسه على الحصر **«ما لم يُوسَّع صراحةً»**. فالتوسيع مأذون، وهو مسجَّل
هنا.

**ولا يُنفَّذ عارياً.** يبقى **منعٌ صلب** لمواضع بعينها، قراءةً وكتابةً، وليس هذا نقضاً لحكمه بل حمايةٌ
لأحكامٍ سابقة له هو:

- `braining-release.jks` و`keystore.properties*` — القيد الصلب رقم ٩ يمنع خروجهما من الجهاز، وفقدان
  مفتاح التوقيع يعني أنّ التطبيق لا يمكن تحديثه على هاتف أي صديق بعد اليوم. **لا شيء يستحقّ هذا.**
- `C:\Users\ASUS\.local\share\opencode\auth.json` — يحوي مفاتيح **تسعة** مزوّدين نصّاً صريحاً. وكيلٌ
  يقرأه يستطيع تسريبها، وهذا يخالف القيد الصلب رقم ٣ (BYOK: المفتاح لا يُسجَّل ولا يُرسَل).
- `.ssh` و`.gnupg` وما يماثلهما من مخازن اعتماد.

**تعديل ٥ أيلول (بعد المراجعة):** أمر المالك بإخراج `C:\Windows` و`Program Files` من قائمة المنع،
ليتمكّن الوكيل من التعامل مع البرامج المثبَّتة. وأبقى الباقي كما هو. والحدّ هنا عمليّ لا سياسيّ:
الكتابة في أيٍّ منهما تحتاج صلاحية مدير، والجسر لا يُشغَّل مرفوع الصلاحية، فالكتابة ستفشل ما لم
يتغيّر ذلك.

ما عدا ذلك: القرص كلّه، قراءةً وكتابةً، تحت الحارسين الآخرين.

### M7-2 · بوّابة الموافقة: عند الإجراء المدمِّر فقط

كما هو منصوص في `BRAINING.md` §7 — حذف، تعديل جماعي، أمر نظام. لا موافقة على كل كتابة.

### M7-3 · المفاتيح: **مفاتيح OpenCode نفسه، وهي مضبوطة سلفاً**

سأل عن ماهيّتها وكيف يجلبها. الجواب: **لا يلزمه جلب شيء.** فُحص جهازه في ٥ أيلول فوُجد
`C:\Users\ASUS\.local\share\opencode\auth.json` يحوي اعتمادات **تسعة** مزوّدين مضبوطة من قبل:
anthropic، openai، google، deepseek، openrouter، huggingface، github-copilot، opencode، opencode-go.
تُدار بأمر `opencode auth` (واسمه البديل `opencode providers`).

**وBraining لا يرى أياً منها ولا يمرّ بها.** الجسر يشغّل OpenCode، وOpenCode يقرأ ملفّه بنفسه. هذا هو
سبب اختيار هذا الخيار: أقلّ سطح تعرّض ممكن — مفتاح لا يمرّ بالتطبيق لا يمكن أن يُسرَّب منه.

### M7-4 · التشغيل يدويّ — مع تصحيح مفهوم، وتناقض يجب أن يعرفه

اختار التشغيل اليدوي، واشترط **ألّا يكون الجسر مرتبطاً بالشبكة المحلية** ليصل إليه وهو بعيد.

**التصحيح:** الشرط محقَّق أصلاً في التصميم. الربط على عنوان Tailscale (`100.117.245.110`)
**ليس شبكة محلية** — Tailscale شبكة خاصة تعمل من أي مكان في الأرض، وهاتفه عليها بالفعل ونشط عبر
مُرحِّل. لا حاجة لفتح منفذ ولا للقرب من الحاسب.

**والتناقض:** لكي يكون الجسر قابلاً للوصول وهو بعيد، **يجب أن يكون شغّالاً**. والتشغيل اليدوي يعني
أنه إن نسي تشغيله قبل خروجه فلا سبيل لتشغيله عن بُعد — إذ لا شيء يستمع. فالقرار مسجَّل كما اختاره،
**على أن يُراجَع فور إثبات الحارسين الأولين**: التشغيل التلقائي مع ويندوز هو ما يجعل الهدف الأصلي
(«أطلب مهمّة وأنا بعيد») يعمل فعلاً. يدويّاً الآن لبناء الثقة، تلقائياً بعدها.

### M8-1..3 · الموقع مُطفأ · الصيغة Markdown · عرض تقرير الانهيار تلقائياً

الموقع **لا يُجمَع إطلاقاً** — لا حتى بدقّة المدينة. والتقارير بصيغة `.md` لأنه هو قارئها الوحيد.
وبعد أي انهيار يُعرض التقرير تلقائياً في التشغيل التالي، مرّة واحدة.

### M8-4 · التقارير تتراكم، **ومعها تقرير جامع واحد** — وهذا هو الطلب الحقيقي

«حتى الانتهاء منها، بحيث يتم تجميع تقارير الفشل كلها بتقرير فشل واحد يعطي ملخصاً موجزاً عن كل تقرير
مع تاريخ التنفيذ والتوقيت والمستخدم وغيرها.»

فالاحتفاظ ليس مدّةً زمنية بل **حتى المعالجة**. ويلزم شيئان:

1. **فهرس جامع** (`reports/index.md`) — سطر واحد لكل عطل: التاريخ، الوقت، مُعرِّف المستخدم، إصدار
   التطبيق، المزوّد، نوع الخطأ، وملخّص بسطر. يُصدَّر ملفاً واحداً للمراجعة والإحصاء.
2. **مُعرِّف مستخدم مجهول** — لأن التطبيق يُوزَّع على الأصدقاء ولا بدّ من تمييز «مَن». يُولَّد
   **رقم عشوائي (UUID) عند أول تشغيل** ويُخزَّن محلياً. **ليس** اسماً ولا بريداً ولا رقم هاتف ولا
   مُعرِّف جهاز إعلانياً: القيد الصلب يقول إنّ الحزمة المنشورة تخلو من أي بيانات شخصية، ورقمٌ عشوائي
   يميّز الجهاز دون أن يعرّف صاحبه. وهو كافٍ تماماً لِما أراده: «تقارير وإحصائيات».

---

## Part 21 · تعليمات ٥ أيلول ٢٠٢٦ — برنامج الإطلاق (L1 → L9)، وثلاثة أسئلة مفتوحة

أملى المالك تسعة بنود لخطّة الإطلاق النهائية وأمر بالالتزام بترتيبها. كُتب لكلٍّ منها موجزٌ مستقلّ في
`docs/launch/` بصيغةٍ يفهمها وكيلٌ لم يرَ المشروع قبلاً، والخطّة الأمّ في `docs/launch/00_LAUNCH_PLAN.md`.
**الترتيب مُلزِم كما أملاه.** ما يلي هو ما سُجِّل كقرار، وما بقي سؤالاً له.

### مُسجَّل كقرار
- **L1-ب · نظام اختبار شامل**: آليّ بلا تدخّل بشري ما أمكن (وحدات، اختبارات مُجهَّزة على الهاتف،
  حلقات قيادة الجهاز عبر adb التي ثبتت في ٥ أيلول)، ثم مصفوفة يدوية واحدة، ثم بروتوكول لتحويل شكوى
  المستخدم إلى إعادة إنتاج فإصلاح.
- **L2 · تدقيق أمني صارم** قبل أي نشر عام، ويُعاد قبل كل إصدار.
- **L3 · دليل PDF شامل** (عربي أولاً ثم إنكليزي) يُولَّد من مصادر التطبيق نفسها، **ونصّ فيديو** يتبع
  ترقيمه حرفياً ليسجّله المالك بنفسه.
- **L4 · ترخيص وشروط وسياسة خصوصية**، ملكيةٌ حصرية باسم **Moustafa Afara**، والاسم في شاشة «حول»
  رابطٌ يفتح `https://t.me/Mustafa_Afara`.
- **L5 · خطّة Google Play تفصيلية.**
- **L6 · موقع ويب** عصري تقني متفاعل بهدوء، على GitHub Pages انسجاماً مع «لا خادم».
- **L7 · وثيقة تقنية كاملة بالعربية البسيطة** من تهيئة البيئة حتى المتاجر — **آخر مراحل البناء** لأنها
  توثّق ما قبلها.
- **L8 · خطّة تسويق كاملة** مجانية ومدفوعة، **ودعمٌ يديره وكلاء** بلا فريق. ويُسجَّل هنا أنّ التطبيق
  نفسه **يتوسّع بلا كلفة** أصلاً: لا خادم ولا حساب ولا كلفة API على المالك (BYOK)، فمئة ألف تثبيت
  تكلّفه ما تكلّفه عشرة. ما يحتاج توسيعاً هو الدعم البشري، وحلّه: الدليل، وتقارير M8 الجامعة، وبوت
  تلغرام يصعّد للمالك ملخّصاً واحداً يومياً.
- **L9 · مقترح كامل للإصدار الثاني**، للعمل عليه لاحقاً.

### ثلاثة أسئلة مفتوحة تحتاج حكم المالك — وقد سُجِّلت توصيةٌ لكلٍّ منها لا قرار

**١ · L1-أ — الوصول عن بُعد إلى هواتف المستخدمين عبر Tailscale.** التوصية: **لا يُبنى**. Tailscale
يصل **الجهاز** لا «التطبيق فقط»، ولا سبيل لحصره؛ و«استخدام التطبيق عن بُعد» يعني باباً خلفياً داخل
النسخة المنشورة يستمع لغير صاحب الهاتف — وهو بالضبط ما يوجد L2 لمنعه، ويُرجَّح أن يُرفَض في مراجعة
Play، ويناقض وعد التطبيق كلّه (لا شيء يخرج من الهاتف)، ويجعل المالك مسؤولاً عن هواتف أصدقائه لو
اختُرق حاسبه. **البديل الذي يحقّق الغاية نفسها** — «الاطّلاع على المشاكل التي تحدث لديهم» — هو
تقرير M8 بضغطة واحدة يرسله المستخدم (ويحمل أكثر مما تُظهره جلسة عن بُعد)، **وجلسة دعم يبدأها
المستخدم** وينهيها هو، تُنتج ملفاً يشاركه، ولا شيء وارد ولا شيء يبقى مفتوحاً.

**٢ · L5-ب — متجر آبل.** التطبيق Kotlin/Compose لأندرويد، **ولا يمكن رفعه إلى App Store بأي
تحويل أو تغليف**. نسخة iOS تعني بناءً ثانياً — الموصى به Compose Multiplatform لأن `core-domain`
كوتلن خالص وينتقل كما هو — وحساب مطوّر آبل (99$/سنة) وجهاز Mac وXcode، وحجم عمل يوازي M1→M4 مجتمعة.
التوصية: **مرحلة مستقلّة مشروطة بنتائج Play أولاً**. يحتاج حكماً: الآن، أم بعد Play، أم لا.

**٣ · L4 — مستودع عامّ بحقوق محفوظة كاملة، أم خاصّ؟** «الملكية الحصرية» تتّفق مع الاثنين، لكن نصّ
الترخيص يختلف. التوصية: **عامّ بحقوق محفوظة** (source-available، ليس مفتوح المصدر) لأنه يلائم ثقةَ
تطبيقٍ يقول «لا شيء يخرج من هاتفك» — فالأصدقاء يرون ذلك بأعينهم. ويُذكَّر: كاتب هذه النصوص ليس
محامياً، ومراجعة محامٍ قبل تقديم Play موصى بها.

### تصحيحات صغيرة سُجِّلت أثناء الكتابة
- Play يشترط **رابط سياسة خصوصية عامّاً** قبل القبول؛ لذلك L4 قبل L5 (وترتيب المالك يحقّق هذا).
- تهجئة الاسم: البريد يستعمل «Moustafa» والطلب «Mustafa» — تُختار تهجئة واحدة لكل النصوص القانونية.
- `android:allowBackup` حالياً true؛ التوصية false لتطبيق BYOK (سؤال L2).

### أحكام المالك — ٥ أيلول ٢٠٢٦، مساءً (تُغلق الأسئلة الثلاثة)
1. **L1-أ:** «افعل كما اقترحت» — **لا وصولَ عن بُعد إلى هواتف المستخدمين.** تُبنى جلسة الدعم التي
   يبدأها المستخدم + تقارير M8. هذا حكمٌ نهائي، ويُقرأ مع L2: لا باب خلفي في النسخة المنشورة.
2. **L5-ب:** آبل **بعد نتائج Play** — مرحلة مستقلّة مشروطة، ولا عمل عليها قبل ذلك.
3. **L4:** **مستودع عامّ بحقوق محفوظة كاملة** (source-available). الترخيص يُكتب على هذا الأساس.
4. **الاسم:** **Moustafa Afara** — كما في البريد — في كل النصوص القانونية وشاشة «حول». رابط تلغرام
   يبقى كما أعطاه: `https://t.me/Mustafa_Afara`.
5. **`allowBackup`:** أُطفئ في اللحظة نفسها (`AndroidManifest.xml`). النتيجة المقصودة: المفاتيح تُدخَل
   من جديد على هاتفٍ جديد ولا تُنقَل.
6. **التسويق:** المالك ينشئ قناة تلغرام اليوم.

### المعرِّف الرسمي للمشروع — ٥ أيلول ٢٠٢٦
اختار المالك **`Braining_AI`** بعد أن وجد `Braining` محجوزاً، وقرّر أن يكون **العلامة التجارية للمشروع
على كل وسائل التواصل** لا اسم قناة تلغرام فحسب.

- القناة: `https://t.me/Braining_AI`.
- **يُميَّز عن رابط المالك الشخصي** `https://t.me/Mustafa_Afara` — وهذا الأخير يبقى بتهجئته كما أعطاه،
  ويظلّ الرابط الذي يفتحه اسمه في شاشة «حول». القناة شيء آخر: هي طريق التحديثات والدعم، ولا ينبغي
  لمستخدمٍ أن يراسل المالك شخصياً ليعرف أنّ نسخة جديدة صدرت.
- يظهر المعرِّف في: شاشة «حول» (L4)، تذييل الموقع وصفحة التواصل (L6)، الدليل وصفحة الأخطاء (L3)،
  وخطّة التسويق (L8).
- **يُحجَز المعرِّف نفسه فوراً** على يوتيوب وX وإنستغرام وتيك توك وGitHub قبل أي منشور إطلاق، ولو بقيت
  الحسابات فارغة شهوراً — معرِّفٌ يأخذه غيرك بعد الإطلاق لا يُشترى.
