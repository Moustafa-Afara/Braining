...
object LocalEndpoint {
    ...
    fun parse(raw: String): Result {
        ...
        if (!isParseable(bare)) return Result.Malformed
        if (!isPrivate(bare)) return Result.NotPrivate(bare)
    ...
    fun baseUrlOrNull(raw: String): String? = (parse(raw) as? Result.Ok)?.url
     ...
     */
    private fun isParseable(host: String): Boolean =
        host.equals("localhost", ignoreCase = true) || isIpv4(host) || isIpv6(host)
     ...
     *
     * **`100.64/10` — carrier-grade NAT — is deliberately NOT here**, and it was, briefly, on the
     * reasoning that Tailscale assigns from it and M6 will need it. That reasoning was wrong in
     * the way that matters: CGNAT is the range a **mobile carrier** hands out, so on a cellular
     * connection those addresses belong to *other subscribers*. A mistyped `100.x` would put a
     * prompt in cleartext on a stranger's device — precisely the outcome this function exists to
     * prevent, and the one range where "private" and "mine" come apart. It comes back with M6,
     * scoped to the bridge, not before it and not by default.
     *
